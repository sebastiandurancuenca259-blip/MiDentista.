  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-20 py-12">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-md overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-500 font-medium">Cargando pacientes desde Supabase...</div>
        ) : patients.length === 0 ? (
          <div className="p-12 text-center text-slate-500 font-medium">No hay registros guardados en la tabla "patients".</div>
        ) : (
                ...
                {patients.map((patient) => (
                  <tr key={patient.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-5 pl-8 font-bold text-[#0f172a]">
                      {patient.full_name || patient.fullName || patient.name || 'Sin Nombre'}
                    </td>
                    ...
                    <td className="p-5">
                      <span className="inline-block bg-[#0ea5e9]/10 text-[#0ea5e9] text-xs font-bold px-3 py-1 rounded-full">
                        {patient.interest || patient.service || 'Consulta General'}
                      </span>
                    </td>
                    <td className="p-5 text-slate-500 text-xs">
                      <div className="flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-slate-400" />
                        <span>{patient.created_at ? new Date(patient.created_at).toLocaleDateString() : 'N/A'}</span>
                      </div>
                    </td>
                  </tr>
                ))}
